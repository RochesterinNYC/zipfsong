...
Solution Notes:
- Kattio IO class was used to speed up the input and parsing process as the Scanner class is too slow.
- Bottlenecking in program seems to be the IO despite not using the Scanner class.

- Algorithmic solution primarily involves the filling of an array with the tracks of the album. Quality for each track is calculated when the track object is created.
- The array is then sorted in descending quality order. The Arrays.sort method used for the sorting is a "modified mergesort" that guarantees n*log(n) runtime. So the sort occurs once and takes nlog(n) time where n is the number of tracks.
- At end of processing and the sort, the top requested tracks are printed (first m elements of array where m is the number of requested songs).